-- Manual, per-sale discount applied at checkout ("desconto na hora").
-- The discount lowers the sale's gross revenue and profit by the same amount
-- (cost is unchanged) and is recorded on the sale for auditing. Sale items keep
-- their nominal per-unit prices, so the per-product price breakdown is unaffected.
...
begin
  ...
  -- Manual discount entered at checkout: never negative, never more than the sale total.
  v_manual_discount := least(
    greatest(coalesce((payload->>'manual_discount')::numeric, 0), 0),
    v_gross_total
  );

  update public.sales
    set
      gross_total = v_gross_total - v_manual_discount,
      cost_total = v_cost_total,
      profit_total = v_profit_total - v_manual_discount,
      discount_total = v_manual_discount
