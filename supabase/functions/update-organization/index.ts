...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (typeof payload?.name === "string") {
      ...
      if (!name) {
        return Response.json(
          { error: "O nome do evento nao pode ficar vazio." },
    ...
    if (Array.isArray(payload?.cashier_names)) {
      const cashierNames = (payload.cashier_names as unknown[])
        .map((item) => String(item ?? "").trim())
        .filter((item) => item.length > 0);
      patch.cashier_names = cashierNames;
    }
